...
public class MazeKeys {
    ...
    public static int minimumStepsToCollectKeys(String[] map) {
        ...
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                ...
                if (cell == 'S') {
                    ...
                    targetKeys |= (1 << ('r' - 'a')); // Set the bit for the exit door
                } else if (cell >= 'q' && cell <= 'r') {
        ...
        boolean[][][] visited = new boolean[rows][cols][1 << 26]; // Adjusted bitmask size
        ...
        while (!queue.isEmpty()) {
            ...
            for (int[] dir : directions) {
                ...
                if (newX >= 0 && newX < rows && newY >= 0 && newY < cols && map[newX].charAt(newY) != 'W') {
                    ...
                    if (cell == 'E' || cell == 'P' || (cell >= 'q' && cell <= 'r') || (cell >= 'Q' && cell <= 'R' && (keys & (1 << (cell - 'A'))) != 0)) {
                        int newKeys = keys;
                        if (cell >= 'q' && cell <= 'r') {
    ...
    public static void main(String[] args) {
        String[] grid = {"SPqPP", "WWWPW", "rPQPR"};
        int result = minimumStepsToCollectKeys(grid);
        System.out.println("Minimum number of moves: " + result); // Output: Adjusted result based on the provided grid
    }
}
